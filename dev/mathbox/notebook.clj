...
;; The return value, `camera`, is a mathbox selection that points to the
;; `<camera />` element. This is similar to how jQuery selections work. You can
;; also select elements with CSS selectors, e.g. finding our camera:
...
;; When you're done, you can do a `mathbox.remove("*")` to empty out the tree
;; and start with a clean slate, or `mathbox.select('cartesian > *').remove()`
;; to empty out the view only.
