...
!0===extensions.has("EXT_texture_filter_anisotropic")&&(supportsMips=extensions.get("EXT_texture_filter_anisotropic"),texture.magFilter===_constants.NearestFilter||texture.minFilter!==_constants.NearestMipmapLinearFilter&&texture.minFilter!==_constants.LinearMipmapLinearFilter||texture.type===_constants.FloatType&&!1===extensions.has("OES_texture_float_linear")||!1===isWebGL2&&texture.type===_constants.HalfFloatType&&!1===extensions.has("OES_texture_half_float_linear")||!(1<texture.anisotropy||properties.get(texture).__currentAnisotropy)||
(_gl.texParameterf(textureType,supportsMips.TEXTURE_MAX_ANISOTROPY_EXT,Math.min(texture.anisotropy,capabilities.getMaxAnisotropy())),properties.get(texture).__currentAnisotropy=texture.anisotropy))}function initTexture(textureProperties,texture){let forceUpload=!1;void 0===textureProperties.__webglInit&&(textureProperties.__webglInit=!0,texture.addEventListener("dispose",onTextureDispose));var source=texture.source;let webglTextures=_sources.get(source);void 0===webglTextures&&(webglTextures={},_sources.set(source,
webglTextures));source=[];source.push(texture.wrapS);source.push(texture.wrapT);source.push(texture.wrapR||0);source.push(texture.magFilter);source.push(texture.minFilter);source.push(texture.anisotropy);source.push(texture.internalFormat);source.push(texture.format);source.push(texture.type);source.push(texture.generateMipmaps);source.push(texture.premultiplyAlpha);source.push(texture.flipY);source.push(texture.unpackAlignment);source.push(texture.encoding);source=source.join();if(source!==textureProperties.__cacheKey){void 0===
webglTextures[source]&&(webglTextures[source]={texture:_gl.createTexture(),usedTimes:0},info.memory.textures++,forceUpload=!0);webglTextures[source].usedTimes++;const webglTexture=webglTextures[textureProperties.__cacheKey];void 0!==webglTexture&&(webglTextures[textureProperties.__cacheKey].usedTimes--,0===webglTexture.usedTimes&&deleteTexture(texture));textureProperties.__cacheKey=source;textureProperties.__webglTexture=webglTextures[source].texture}return forceUpload}function uploadTexture(textureProperties,
texture,slot){let textureType=_gl.TEXTURE_2D;if(texture.isDataArrayTexture||texture.isCompressedArrayTexture)textureType=_gl.TEXTURE_2D_ARRAY;texture.isData3DTexture&&(textureType=_gl.TEXTURE_3D);var forceUpload=initTexture(textureProperties,texture);const source=texture.source;state.bindTexture(textureType,textureProperties.__webglTexture,_gl.TEXTURE0+slot);const sourceProperties=properties.get(source);if(source.version!==sourceProperties.__version||!0===forceUpload){state.activeTexture(_gl.TEXTURE0+
slot);_gl.pixelStorei(_gl.UNPACK_FLIP_Y_WEBGL,texture.flipY);_gl.pixelStorei(_gl.UNPACK_PREMULTIPLY_ALPHA_WEBGL,texture.premultiplyAlpha);_gl.pixelStorei(_gl.UNPACK_ALIGNMENT,texture.unpackAlignment);_gl.pixelStorei(_gl.UNPACK_COLORSPACE_CONVERSION_WEBGL,_gl.NONE);slot=isWebGL2?!1:texture.wrapS!==_constants.ClampToEdgeWrapping||texture.wrapT!==_constants.ClampToEdgeWrapping||texture.minFilter!==_constants.NearestFilter&&texture.minFilter!==_constants.LinearFilter;slot=slot&&!1===isPowerOfTwo(texture.image);
var image=resizeImage(texture.image,slot,!1,maxTextureSize);image=verifyColorSpace(texture,image);slot=isPowerOfTwo(image)||isWebGL2;const glFormat=utils.convert(texture.format,texture.encoding);let glType=utils.convert(texture.type),glInternalFormat=getInternalFormat(texture.internalFormat,glFormat,glType,texture.encoding,texture.isVideoTexture);setTextureParameters(textureType,texture,slot);var mipmaps=texture.mipmaps,useTexStorage=isWebGL2&&!0!==texture.isVideoTexture;const allocateMemory=void 0===
sourceProperties.__version||!0===forceUpload;forceUpload=getMipLevels(texture,image,slot);if(texture.isDepthTexture)glInternalFormat=_gl.DEPTH_COMPONENT,isWebGL2?glInternalFormat=texture.type===_constants.FloatType?_gl.DEPTH_COMPONENT32F:texture.type===_constants.UnsignedIntType?_gl.DEPTH_COMPONENT24:texture.type===_constants.UnsignedInt248Type?_gl.DEPTH24_STENCIL8:_gl.DEPTH_COMPONENT16:texture.type===_constants.FloatType&&console.error("WebGLRenderer: Floating point depth texture requires WebGL2."),
texture.format===_constants.DepthFormat&&glInternalFormat===_gl.DEPTH_COMPONENT&&texture.type!==_constants.UnsignedShortType&&texture.type!==_constants.UnsignedIntType&&(console.warn("THREE.WebGLRenderer: Use UnsignedShortType or UnsignedIntType for DepthFormat DepthTexture."),texture.type=_constants.UnsignedIntType,glType=utils.convert(texture.type)),texture.format===_constants.DepthStencilFormat&&glInternalFormat===_gl.DEPTH_COMPONENT&&(glInternalFormat=_gl.DEPTH_STENCIL,texture.type!==_constants.UnsignedInt248Type&&
(console.warn("THREE.WebGLRenderer: Use UnsignedInt248Type for DepthStencilFormat DepthTexture."),texture.type=_constants.UnsignedInt248Type,glType=utils.convert(texture.type))),allocateMemory&&(useTexStorage?state.texStorage2D(_gl.TEXTURE_2D,1,glInternalFormat,image.width,image.height):state.texImage2D(_gl.TEXTURE_2D,0,glInternalFormat,image.width,image.height,0,glFormat,glType,null));else if(texture.isDataTexture)if(0<mipmaps.length&&slot){useTexStorage&&allocateMemory&&state.texStorage2D(_gl.TEXTURE_2D,
...
texture.format!==_constants.RGBAFormat?null!==glFormat?useTexStorage?state.compressedTexSubImage3D(_gl.TEXTURE_2D_ARRAY,i,0,0,0,forceUpload.width,forceUpload.height,image.depth,glFormat,forceUpload.data,0,0):state.compressedTexImage3D(_gl.TEXTURE_2D_ARRAY,i,glInternalFormat,forceUpload.width,forceUpload.height,image.depth,0,forceUpload.data,0,0):console.warn("THREE.WebGLRenderer: Attempt to load unsupported compressed texture format in .uploadTexture()"):useTexStorage?state.texSubImage3D(_gl.TEXTURE_2D_ARRAY,
i,0,0,0,forceUpload.width,forceUpload.height,image.depth,glFormat,glType,forceUpload.data):state.texImage3D(_gl.TEXTURE_2D_ARRAY,i,glInternalFormat,forceUpload.width,forceUpload.height,image.depth,0,glFormat,glType,forceUpload.data)}else{useTexStorage&&allocateMemory&&state.texStorage2D(_gl.TEXTURE_2D,forceUpload,glInternalFormat,mipmaps[0].width,mipmaps[0].height);for(let i=0,il=mipmaps.length;i<il;i++)forceUpload=mipmaps[i],texture.format!==_constants.RGBAFormat?null!==glFormat?useTexStorage?state.compressedTexSubImage2D(_gl.TEXTURE_2D,
...
renderTarget.depthTexture.image.width===renderTarget.width&&renderTarget.depthTexture.image.height===renderTarget.height||(renderTarget.depthTexture.image.width=renderTarget.width,renderTarget.depthTexture.image.height=renderTarget.height,renderTarget.depthTexture.needsUpdate=!0);setTexture2D(renderTarget.depthTexture,0);renderTargetProperties=properties.get(renderTarget.depthTexture).__webglTexture;isCube=getRenderTargetSamples(renderTarget);if(renderTarget.depthTexture.format===_constants.DepthFormat)useMultisampledRTT(renderTarget)?
multisampledRTTExt.framebufferTexture2DMultisampleEXT(_gl.FRAMEBUFFER,_gl.DEPTH_ATTACHMENT,_gl.TEXTURE_2D,renderTargetProperties,0,isCube):_gl.framebufferTexture2D(_gl.FRAMEBUFFER,_gl.DEPTH_ATTACHMENT,_gl.TEXTURE_2D,renderTargetProperties,0);else if(renderTarget.depthTexture.format===_constants.DepthStencilFormat)useMultisampledRTT(renderTarget)?multisampledRTTExt.framebufferTexture2DMultisampleEXT(_gl.FRAMEBUFFER,_gl.DEPTH_STENCIL_ATTACHMENT,_gl.TEXTURE_2D,renderTargetProperties,0,isCube):_gl.framebufferTexture2D(_gl.FRAMEBUFFER,
...
texture.generateMipmaps=!1):image=_ImageUtils.ImageUtils.sRGBToLinear(image):format===_constants.RGBAFormat&&type===_constants.UnsignedByteType||console.warn("THREE.WebGLTextures: sRGB encoded textures have to use RGBAFormat and UnsignedByteType."):console.error("THREE.WebGLTextures: Unsupported texture encoding:",encoding));return image}const isWebGL2=capabilities.isWebGL2,maxTextures=capabilities.maxTextures,maxCubemapSize=capabilities.maxCubemapSize,maxTextureSize=capabilities.maxTextureSize,maxSamples=
capabilities.maxSamples,multisampledRTTExt=extensions.has("WEBGL_multisampled_render_to_texture")?extensions.get("WEBGL_multisampled_render_to_texture"):null,supportsInvalidateFramebuffer="undefined"===typeof navigator?!1:/OculusBrowser/g.test(navigator.userAgent),_videoTextures=new WeakMap;let _canvas;const _sources=new WeakMap;let useOffscreenCanvas=!1;try{useOffscreenCanvas="undefined"!==typeof OffscreenCanvas&&null!==(new OffscreenCanvas(1,1)).getContext("2d")}catch(err){}let textureUnits=0;const wrappingToGL=
{[_constants.RepeatWrapping]:_gl.REPEAT,[_constants.ClampToEdgeWrapping]:_gl.CLAMP_TO_EDGE,[_constants.MirroredRepeatWrapping]:_gl.MIRRORED_REPEAT},filterToGL={[_constants.NearestFilter]:_gl.NEAREST,[_constants.NearestMipmapNearestFilter]:_gl.NEAREST_MIPMAP_NEAREST,[_constants.NearestMipmapLinearFilter]:_gl.NEAREST_MIPMAP_LINEAR,[_constants.LinearFilter]:_gl.LINEAR,[_constants.LinearMipmapNearestFilter]:_gl.LINEAR_MIPMAP_NEAREST,[_constants.LinearMipmapLinearFilter]:_gl.LINEAR_MIPMAP_LINEAR};this.allocateTextureUnit=
function(){const textureUnit=textureUnits;textureUnit>=maxTextures&&console.warn("THREE.WebGLTextures: Trying to use "+textureUnit+" texture units while this GPU supports only "+maxTextures);textureUnits+=1;return textureUnit};this.resetTextureUnits=function(){textureUnits=0};this.setTexture2D=setTexture2D;this.setTexture2DArray=function(texture,slot){const textureProperties=properties.get(texture);0<texture.version&&textureProperties.__version!==texture.version?uploadTexture(textureProperties,texture,
slot):state.bindTexture(_gl.TEXTURE_2D_ARRAY,textureProperties.__webglTexture,_gl.TEXTURE0+slot)};this.setTexture3D=function(texture,slot){const textureProperties=properties.get(texture);0<texture.version&&textureProperties.__version!==texture.version?uploadTexture(textureProperties,texture,slot):state.bindTexture(_gl.TEXTURE_3D,textureProperties.__webglTexture,_gl.TEXTURE0+slot)};this.setTextureCube=function(texture,slot){const textureProperties=properties.get(texture);if(0<texture.version&&textureProperties.__version!==
texture.version){if(6===texture.image.length){var forceUpload=initTexture(textureProperties,texture),source=texture.source;state.bindTexture(_gl.TEXTURE_CUBE_MAP,textureProperties.__webglTexture,_gl.TEXTURE0+slot);var sourceProperties=properties.get(source);if(source.version!==sourceProperties.__version||!0===forceUpload){state.activeTexture(_gl.TEXTURE0+slot);_gl.pixelStorei(_gl.UNPACK_FLIP_Y_WEBGL,texture.flipY);_gl.pixelStorei(_gl.UNPACK_PREMULTIPLY_ALPHA_WEBGL,texture.premultiplyAlpha);_gl.pixelStorei(_gl.UNPACK_ALIGNMENT,
