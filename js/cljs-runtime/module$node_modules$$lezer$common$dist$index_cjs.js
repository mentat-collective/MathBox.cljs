...
if(!node.nextSibling())return null==after?result:[]}}function matchNodeContext(node,context,i=context.length-1){for(node=node.parent;0<=i;node=node.parent){if(!node)return!1;if(!node.type.isAnonymous){if(context[i]&&context[i]!=node.name)return!1;i--}}return!0}function hasChild(tree){return tree.children.some(ch=>ch instanceof TreeBuffer||!ch.type.isAnonymous||hasChild(ch))}function buildTree(data){function takeNode(parentStart,minPos,children,positions,inRepeat){let {id,start,end,size}=cursor;for(var lookAheadAtStart=
lookAhead;0>size;){cursor.next();if(-1==size){children.push(reused[id]);positions.push(start-parentStart);return}if(-3==size){contextHash=id;return}if(-4==size){lookAhead=id;return}throw new RangeError(`Unrecognized record size: ${size}`);}var type=types[id],startPos=start-parentStart,JSCompiler_temp;if(JSCompiler_temp=end-start<=maxBufferLength){var buffer=cursor.pos-minPos;minPos=cursor.fork();var start$jscomp$0=JSCompiler_temp=0;let skip=0,minStart=minPos.end-maxBufferLength,result={size:0,start:0,
skip:0};a:for(let minPos$jscomp$0=minPos.pos-buffer;minPos.pos>minPos$jscomp$0;){let nodeSize=minPos.size;if(minPos.id==inRepeat&&0<=nodeSize){result.size=JSCompiler_temp;result.start=start$jscomp$0;result.skip=skip;skip+=4;JSCompiler_temp+=4;minPos.next();continue}let startPos=minPos.pos-nodeSize;if(0>nodeSize||startPos<minPos$jscomp$0||minPos.start<minStart)break;let localSkipped=minPos.id>=minRepeatType?4:0,nodeStart=minPos.start;for(minPos.next();minPos.pos>startPos;){if(0>minPos.size)if(-3==
minPos.size)localSkipped+=4;else break a;else minPos.id>=minRepeatType&&(localSkipped+=4);minPos.next()}start$jscomp$0=nodeStart;JSCompiler_temp+=nodeSize;skip+=localSkipped}if(0>inRepeat||JSCompiler_temp==buffer)result.size=JSCompiler_temp,result.start=start$jscomp$0,result.skip=skip;JSCompiler_temp=buffer=4<result.size?result:void 0}if(JSCompiler_temp){startPos=new Uint16Array(buffer.size-buffer.skip);type=cursor.pos-buffer.size;for(lookAheadAtStart=startPos.length;cursor.pos>type;)lookAheadAtStart=
copyToBuffer(buffer.start,startPos,lookAheadAtStart);type=new TreeBuffer(startPos,end-buffer.start,nodeSet);startPos=buffer.start-parentStart}else{buffer=cursor.pos-size;cursor.next();parentStart=[];inRepeat=[];minPos=id>=minRepeatType?id:-1;JSCompiler_temp=0;for(start$jscomp$0=end;cursor.pos>buffer;)0<=minPos&&cursor.id==minPos&&0<=cursor.size?(cursor.end<=start$jscomp$0-maxBufferLength&&(makeRepeatLeaf(parentStart,inRepeat,start,JSCompiler_temp,cursor.end,start$jscomp$0,minPos,lookAheadAtStart),
JSCompiler_temp=parentStart.length,start$jscomp$0=cursor.end),cursor.next()):takeNode(start,buffer,parentStart,inRepeat,minPos);0<=minPos&&0<JSCompiler_temp&&JSCompiler_temp<parentStart.length&&makeRepeatLeaf(parentStart,inRepeat,start,JSCompiler_temp,start,start$jscomp$0,minPos,lookAheadAtStart);parentStart.reverse();inRepeat.reverse();-1<minPos&&0<JSCompiler_temp?(lookAheadAtStart=makeBalanced(type),type=balanceRange(type,parentStart,inRepeat,0,parentStart.length,0,end-start,lookAheadAtStart,lookAheadAtStart)):
type=makeTree(type,parentStart,inRepeat,end-start,lookAheadAtStart-end)}children.push(type);positions.push(startPos)}function makeBalanced(type){return(children,positions,length)=>{let lookAhead=0,lastI=children.length-1,last,lookAheadProp;if(0<=lastI&&(last=children[lastI])instanceof Tree){if(!lastI&&last.type==type&&last.length==length)return last;if(lookAheadProp=last.prop(NodeProp.lookAhead))lookAhead=positions[lastI]+last.length+lookAheadProp}return makeTree(type,children,positions,length,lookAhead)}}
function makeRepeatLeaf(children,positions,base,i,from,to,type,lookAhead){let localChildren=[],localPositions=[];for(;children.length>i;)localChildren.push(children.pop()),localPositions.push(positions.pop()+base-from);children.push(makeTree(nodeSet.types[type],localChildren,localPositions,to-from,lookAhead-to));positions.push(from-base)}function makeTree(type,children,positions,length,lookAhead=0,props){if(contextHash){let pair=[NodeProp.contextHash,contextHash];props=props?[pair].concat(props):
[pair]}25<lookAhead&&(lookAhead=[NodeProp.lookAhead,lookAhead],props=props?[lookAhead].concat(props):[lookAhead]);return new Tree(type,children,positions,length,props)}function copyToBuffer(bufferStart,buffer,index){let {id,start,end,size}=cursor;cursor.next();if(0<=size&&id<minRepeatType){let startIndex=index;if(4<size){let endPos=cursor.pos-(size-4);for(;cursor.pos>endPos;)index=copyToBuffer(bufferStart,buffer,index)}buffer[--index]=startIndex;buffer[--index]=end-bufferStart;buffer[--index]=start-
bufferStart;buffer[--index]=id}else-3==size?contextHash=id:-4==size&&(lookAhead=id);return index}var _a;let {buffer:buffer$jscomp$0,nodeSet,maxBufferLength=1024,reused=[],minRepeatType=nodeSet.types.length}=data,cursor=Array.isArray(buffer$jscomp$0)?new FlatBufferCursor(buffer$jscomp$0,buffer$jscomp$0.length):buffer$jscomp$0,types=nodeSet.types,contextHash=0,lookAhead=0,children$jscomp$0=[],positions$jscomp$0=[];for(;0<cursor.pos;)takeNode(data.start||0,data.bufferStart||0,children$jscomp$0,positions$jscomp$0,
-1);let length$jscomp$0=null!==(_a=data.length)&&void 0!==_a?_a:children$jscomp$0.length?positions$jscomp$0[0]+children$jscomp$0[0].length:0;return new Tree(types[data.topID],children$jscomp$0.reverse(),positions$jscomp$0.reverse(),length$jscomp$0)}function nodeSize(balanceType,node){if(!balanceType.isAnonymous||node instanceof TreeBuffer||node.type!=balanceType)return 1;let size=nodeSizeCache.get(node);if(null==size){size=1;for(let child of node.children){if(child.type!=balanceType||!(child instanceof
Tree)){size=1;break}size+=nodeSize(balanceType,child)}nodeSizeCache.set(node,size)}return size}function balanceRange(balanceType,children$jscomp$0,positions$jscomp$0,from$jscomp$0,to$jscomp$0,start,length,mkTop,mkTree){function divide(children,positions,from,to,offset){for(;from<to;){let groupFrom=from;var groupStart=positions[from];let groupSize=nodeSize(balanceType,children[from]);for(from++;from<to;from++){let nextSize=nodeSize(balanceType,children[from]);if(groupSize+nextSize>=maxChild)break;
groupSize+=nextSize}if(from==groupFrom+1){if(groupSize>maxChild){groupStart=children[groupFrom];divide(groupStart.children,groupStart.positions,0,groupStart.children.length,positions[groupFrom]+offset);continue}localChildren.push(children[groupFrom])}else localChildren.push(balanceRange(balanceType,children,positions,groupFrom,from,groupStart,positions[from-1]+children[from-1].length-groupStart,null,mkTree));localPositions.push(groupStart+offset-start)}}let total=0;for(let i=from$jscomp$0;i<to$jscomp$0;i++)total+=
nodeSize(balanceType,children$jscomp$0[i]);let maxChild=Math.ceil(1.5*total/8),localChildren=[],localPositions=[];divide(children$jscomp$0,positions$jscomp$0,from$jscomp$0,to$jscomp$0,0);return(mkTop||mkTree)(localChildren,localPositions,length)}function checkCover(covered,from,to){for(let range of covered){if(range.from>=to)break;if(range.to>from)return range.from<=from&&range.to>=to?2:1}return 0}function materialize(cursor){function split(startI,endI,type,innerOffset,length){for(var i=startI;b[i+
2]+off<=node.from;)i=b[i+3];let children=[],positions=[];if(startI<i){var from=buf.buffer[startI+1];children.push(buf.slice(startI,i,from));positions.push(from-innerOffset)}startI=b[i+1];from=b[i+2];children.push(startI+off==node.from&&from+off==node.to&&b[i]==node.type.id?node.toTree():split(i+4,b[i+3],buf.set.types[b[i]],startI,from-startI));positions.push(startI-innerOffset);i=b[i+3];i<endI&&(startI=buf.buffer[i+1],children.push(buf.slice(i,endI,startI)),positions.push(startI-innerOffset));return new Tree(type,
children,positions,length)}let {node}=cursor,depth=0;do cursor.parent(),depth++;while(!cursor.tree);var i$jscomp$0=0;let base=cursor.tree,off=0;for(;!(off=base.positions[i$jscomp$0]+cursor.from,off<=node.from&&off+base.children[i$jscomp$0].length>=node.to);i$jscomp$0++);let buf=base.children[i$jscomp$0],b=buf.buffer;base.children[i$jscomp$0]=split(0,b.length,NodeType.none,0,buf.length);for(i$jscomp$0=0;i$jscomp$0<=depth;i$jscomp$0++)cursor.childAfter(node.from)}function punchRanges(outer,ranges){let copy=
null,current=ranges;for(let i=1,j=0;i<outer.length;i++){let gapFrom=outer[i-1].to,gapTo=outer[i].from;for(;j<current.length;j++){let r=current[j];if(r.from>=gapTo)break;r.to<=gapFrom||(copy||(current=copy=ranges.slice()),r.from<gapFrom?(copy[j]=new Range(r.from,gapFrom),r.to>gapTo&&copy.splice(j+1,0,new Range(gapTo,r.to))):r.to>gapTo?copy[j--]=new Range(gapTo,r.to):copy.splice(j--,1))}}return current}function findCoverChanges(a,b,from,to){let iA=0,iB=0,inA=!1,inB=!1;var pos=-1E9;let result=[];for(;;){let nextA=
iA==a.length?1E9:inA?a[iA].to:a[iA].from,nextB=iB==b.length?1E9:inB?b[iB].to:b[iB].from;if(inA!=inB){pos=Math.max(pos,from);let end=Math.min(nextA,nextB,to);pos<end&&result.push(new Range(pos,end))}pos=Math.min(nextA,nextB);if(1E9==pos)break;nextA==pos&&(inA?(inA=!1,iA++):inA=!0);nextB==pos&&(inB?(inB=!1,iB++):inB=!0)}return result}function enterFragments(mounts,ranges){let result=[];for(let {pos:pos$jscomp$0,mount,frag}of mounts){mounts=pos$jscomp$0+(mount.overlay?mount.overlay[0].from:0);var endPos=
mounts+mount.tree.length,from=Math.max(frag.from,mounts);let to=Math.min(frag.to,endPos);if(mount.overlay){endPos=mount.overlay.map(r=>new Range(r.from+pos$jscomp$0,r.to+pos$jscomp$0));endPos=findCoverChanges(ranges,endPos,from,to);for(let i=0,pos=from;;i++){let end=(from=i==endPos.length)?to:endPos[i].from;end>pos&&result.push(new TreeFragment(pos,end,mount.tree,-mounts,frag.from>=pos||frag.openStart,frag.to<=end||frag.openEnd));if(from)break;pos=endPos[i].to}}else result.push(new TreeFragment(from,
to,mount.tree,-mounts,frag.from>=mounts||frag.openStart,frag.to<=endPos||frag.openEnd))}return result}Object.defineProperty(exports,"__esModule",{value:!0});let nextPropID=0;class Range{constructor(from,to){this.from=from;this.to=to}}class NodeProp{constructor(config={}){this.id=nextPropID++;this.perNode=!!config.perNode;this.deserialize=config.deserialize||(()=>{throw Error("This node type doesn't define a deserialize function");})}add(match){if(this.perNode)throw new RangeError("Can't add per-node props to node types");
"function"!=typeof match&&(match=NodeType.match(match));return type=>{type=match(type);return void 0===type?null:[this,type]}}}NodeProp.closedBy=new NodeProp({deserialize:str=>str.split(" ")});NodeProp.openedBy=new NodeProp({deserialize:str=>str.split(" ")});NodeProp.group=new NodeProp({deserialize:str=>str.split(" ")});NodeProp.contextHash=new NodeProp({perNode:!0});NodeProp.lookAhead=new NodeProp({perNode:!0});NodeProp.mounted=new NodeProp({perNode:!0});class MountedTree{constructor(tree,overlay,
parser){this.tree=tree;this.overlay=overlay;this.parser=parser}}const noProps=Object.create(null);class NodeType{constructor(name,props,id,flags=0){this.name=name;this.props=props;this.id=id;this.flags=flags}static define(spec){let props=spec.props&&spec.props.length?Object.create(null):noProps,type=new NodeType(spec.name||"",props,spec.id,(spec.top?1:0)|(spec.skipped?2:0)|(spec.error?4:0)|(null==spec.name?8:0));if(spec.props)for(let src of spec.props)if(Array.isArray(src)||(src=src(type)),src){if(src[0].perNode)throw new RangeError("Can't store a per-node prop on a node type");
props[src[0].id]=src[1]}return type}prop(prop){return this.props[prop.id]}get isTop(){return 0<(this.flags&1)}get isSkipped(){return 0<(this.flags&2)}get isError(){return 0<(this.flags&4)}get isAnonymous(){return 0<(this.flags&8)}is(name){if("string"==typeof name){if(this.name==name)return!0;let group=this.prop(NodeProp.group);return group?-1<group.indexOf(name):!1}return this.id==name}static match(map){let direct=Object.create(null);for(let prop in map)for(let name of prop.split(" "))direct[name]=
map[prop];return node=>{for(let groups=node.prop(NodeProp.group),i=-1;i<(groups?groups.length:0);i++){let found=direct[0>i?node.name:groups[i]];if(found)return found}}}}NodeType.none=new NodeType("",Object.create(null),0,8);class NodeSet{constructor(types){this.types=types;for(let i=0;i<types.length;i++)if(types[i].id!=i)throw new RangeError("Node type ids should correspond to array positions when creating a node set");}extend(...props){let newTypes=[];for(let type of this.types){let newProps=null;
for(let source of props){let add=source(type);add&&(newProps||(newProps=Object.assign({},type.props)),newProps[add[0].id]=add[1])}newTypes.push(newProps?new NodeType(type.name,newProps,type.id,type.flags):type)}return new NodeSet(newTypes)}}const CachedNode=new WeakMap,CachedInnerNode=new WeakMap;exports.IterMode=void 0;(function(IterMode){IterMode[IterMode.ExcludeBuffers=1]="ExcludeBuffers";IterMode[IterMode.IncludeAnonymous=2]="IncludeAnonymous";IterMode[IterMode.IgnoreMounts=4]="IgnoreMounts";
IterMode[IterMode.IgnoreOverlays=8]="IgnoreOverlays"})(exports.IterMode||(exports.IterMode={}));class Tree{constructor(type,children,positions,length,props){this.type=type;this.children=children;this.positions=positions;this.length=length;this.props=null;if(props&&props.length){this.props=Object.create(null);for(let [prop,value]of props)this.props["number"==typeof prop?prop:prop.id]=value}}toString(){var mounted=this.prop(NodeProp.mounted);if(mounted&&!mounted.overlay)return mounted.tree.toString();
mounted="";for(let ch of this.children){let str=ch.toString();str&&(mounted&&(mounted+=","),mounted+=str)}return this.type.name?(/\W/.test(this.type.name)&&!this.type.isError?JSON.stringify(this.type.name):this.type.name)+(mounted.length?"("+mounted+")":""):mounted}cursor(mode=0){return new TreeCursor(this.topNode,mode)}cursorAt(pos,side=0,mode){mode=CachedNode.get(this)||this.topNode;mode=new TreeCursor(mode);mode.moveTo(pos,side);CachedNode.set(this,mode._tree);return mode}get topNode(){return new TreeNode(this,
0,0,null)}resolve(pos,side=0){pos=resolveNode(CachedNode.get(this)||this.topNode,pos,side,!1);CachedNode.set(this,pos);return pos}resolveInner(pos,side=0){pos=resolveNode(CachedInnerNode.get(this)||this.topNode,pos,side,!0);CachedInnerNode.set(this,pos);return pos}iterate(spec){let {enter,leave,from=0,to=this.length}=spec;for(spec=this.cursor((spec.mode||0)|exports.IterMode.IncludeAnonymous);;){let entered=!1;if(spec.from<=to&&spec.to>=from&&(spec.type.isAnonymous||!1!==enter(spec))){if(spec.firstChild())continue;
entered=!0}for(;;){entered&&leave&&!spec.type.isAnonymous&&leave(spec);if(spec.nextSibling())break;if(!spec.parent())return;entered=!0}}}prop(prop){return prop.perNode?this.props?this.props[prop.id]:void 0:this.type.prop(prop)}get propValues(){let result=[];if(this.props)for(let id in this.props)result.push([+id,this.props[id]]);return result}balance(config={}){return 8>=this.children.length?this:balanceRange(NodeType.none,this.children,this.positions,0,this.children.length,0,this.length,(children,
positions,length)=>new Tree(this.type,children,positions,length,this.propValues),config.makeTree||((children,positions,length)=>new Tree(NodeType.none,children,positions,length)))}static build(data){return buildTree(data)}}Tree.empty=new Tree(NodeType.none,[],[],0);class FlatBufferCursor{constructor(buffer,index){this.buffer=buffer;this.index=index}get id(){return this.buffer[this.index-4]}get start(){return this.buffer[this.index-3]}get end(){return this.buffer[this.index-2]}get size(){return this.buffer[this.index-
