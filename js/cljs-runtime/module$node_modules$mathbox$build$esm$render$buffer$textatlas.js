...
renderer.match(/Chrome/)&&renderer.match(/OS X/)&&(this.gamma=.5));this.scratchW=this.scratchH=0;this.build(options)}build(options){super.build(options);options=16;options=this.size;options+=4+2*Math.min(1,this.outline);const maxWidth=32*options,canvas=document.createElement("canvas");canvas.width=maxWidth;canvas.height=options;var font=this.font.map(str=>`${str.replace(/(['"\\])/g,"\\$1")}`).join(", ");const context=canvas.getContext("2d",{willReadFrequently:!0});context.font=`${this.style} ${this.variant} ${this.weight} ${this.size}px ${font}`;
