...
return this}i=this.children.indexOf(object);-1!==i&&(object.parent=null,this.children.splice(i,1),object.dispatchEvent(_removedEvent));return this}removeFromParent(){const parent=this.parent;null!==parent&&parent.remove(this);return this}clear(){for(let i=0;i<this.children.length;i++){const object=this.children[i];object.parent=null;object.dispatchEvent(_removedEvent)}this.children.length=0;return this}attach(object){this.updateWorldMatrix(!0,!1);_m1.copy(this.matrixWorld).invert();null!==object.parent&&
(object.parent.updateWorldMatrix(!0,!1),_m1.multiply(object.parent.matrixWorld));object.applyMatrix4(_m1);this.add(object);object.updateWorldMatrix(!1,!0);return this}getObjectById(id){return this.getObjectByProperty("id",id)}getObjectByName(name){return this.getObjectByProperty("name",name)}getObjectByProperty(name,value){if(this[name]===value)return this;for(let i=0,l=this.children.length;i<l;i++){const object=this.children[i].getObjectByProperty(name,value);if(void 0!==object)return object}}getObjectsByProperty(name,
value){let result=[];this[name]===value&&result.push(this);for(let i=0,l=this.children.length;i<l;i++){const childResult=this.children[i].getObjectsByProperty(name,value);0<childResult.length&&(result=result.concat(childResult))}return result}getWorldPosition(target){this.updateWorldMatrix(!0,!1);return target.setFromMatrixPosition(this.matrixWorld)}getWorldQuaternion(target){this.updateWorldMatrix(!0,!1);this.matrixWorld.decompose(_position,target,_scale);return target}getWorldScale(target){this.updateWorldMatrix(!0,
!1);this.matrixWorld.decompose(_position,_quaternion,target);return target}getWorldDirection(target){this.updateWorldMatrix(!0,!1);const e=this.matrixWorld.elements;return target.set(e[8],e[9],e[10]).normalize()}raycast(){}traverse(callback){callback(this);const children=this.children;for(let i=0,l=children.length;i<l;i++)children[i].traverse(callback)}traverseVisible(callback){if(!1!==this.visible){callback(this);var children=this.children;for(let i=0,l=children.length;i<l;i++)children[i].traverseVisible(callback)}}traverseAncestors(callback){const parent=
...
return this}}exports.Object3D=Object3D;Object3D.DefaultUp=new _Vector.Vector3(0,1,0);Object3D.DefaultMatrixAutoUpdate=!0;Object3D.DefaultMatrixWorldAutoUpdate=!0}
