...
onSessionEvent);session.addEventListener("squeezestart",onSessionEvent);session.addEventListener("squeezeend",onSessionEvent);session.addEventListener("end",onSessionEnd);session.addEventListener("inputsourceschange",onInputSourcesChange);!0!==attributes.xrCompatible&&await gl.makeXRCompatible();if(void 0===session.renderState.layers||!1===renderer.capabilities.isWebGL2)glBaseLayer=new XRWebGLLayer(session,gl,{antialias:void 0===session.renderState.layers?attributes.antialias:!0,alpha:attributes.alpha,
depth:attributes.depth,stencil:attributes.stencil,framebufferScaleFactor}),session.updateRenderState({baseLayer:glBaseLayer}),newRenderTarget=new _WebGLRenderTarget.WebGLRenderTarget(glBaseLayer.framebufferWidth,glBaseLayer.framebufferHeight,{format:_constants.RGBAFormat,type:_constants.UnsignedByteType,encoding:renderer.outputEncoding,stencilBuffer:attributes.stencil});else{let depthType=value=null;var glDepthFormat=null;attributes.depth&&(glDepthFormat=attributes.stencil?gl.DEPTH24_STENCIL8:gl.DEPTH_COMPONENT24,
value=attributes.stencil?_constants.DepthStencilFormat:_constants.DepthFormat,depthType=attributes.stencil?_constants.UnsignedInt248Type:_constants.UnsignedIntType);glDepthFormat={colorFormat:gl.RGBA8,depthFormat:glDepthFormat,scaleFactor:framebufferScaleFactor};glBinding=new XRWebGLBinding(session,gl);glProjLayer=glBinding.createProjectionLayer(glDepthFormat);session.updateRenderState({layers:[glProjLayer]});newRenderTarget=new _WebGLRenderTarget.WebGLRenderTarget(glProjLayer.textureWidth,glProjLayer.textureHeight,
{format:_constants.RGBAFormat,type:_constants.UnsignedByteType,depthTexture:new _DepthTexture.DepthTexture(glProjLayer.textureWidth,glProjLayer.textureHeight,depthType,void 0,void 0,void 0,void 0,void 0,void 0,value),stencilBuffer:attributes.stencil,encoding:renderer.outputEncoding,samples:attributes.antialias?4:0});renderer.properties.get(newRenderTarget).__ignoreDepthValues=glProjLayer.ignoreDepthValues}newRenderTarget.isXRRenderTarget=!0;this.setFoveation(1);customReferenceSpace=null;referenceSpace=
await session.requestReferenceSpace(referenceSpaceType);animation.setContext(session);animation.start();scope.isPresenting=!0;scope.dispatchEvent({type:"sessionstart"})}};const cameraLPos=new _Vector.Vector3,cameraRPos=new _Vector.Vector3;this.updateCamera=function(camera){if(null!==session){cameraVR.near=cameraR.near=cameraL.near=camera.near;cameraVR.far=cameraR.far=cameraL.far=camera.far;if(_currentDepthNear!==cameraVR.near||_currentDepthFar!==cameraVR.far)session.updateRenderState({depthNear:cameraVR.near,
...
planesLastChangedTimes.delete(plane),scope.dispatchEvent({type:"planeremoved",data:plane});for(const plane of frame.detectedPlanes)planes.has(plane)?(plane$jscomp$0=planesLastChangedTimes.get(plane),plane.lastChangedTime>plane$jscomp$0&&(planesLastChangedTimes.set(plane,plane.lastChangedTime),scope.dispatchEvent({type:"planechanged",data:plane}))):(planes.add(plane),planesLastChangedTimes.set(plane,frame.lastChangedTime),scope.dispatchEvent({type:"planeadded",data:plane}))}xrFrame=null});this.setAnimationLoop=
function(callback){onAnimationFrameCallback=callback};this.dispose=function(){}}}exports.WebXRManager=WebXRManager}
