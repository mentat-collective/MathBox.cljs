...
cssVar.registerCssVariable(name):canAnimateNatively=!1);valueIsTransform&&!featureDetection.supports.linearEasing()&&(utils.isFunction(easing$1)||utils.isEasingList(easing$1)&&easing$1.some(utils.isFunction))&&(canAnimateNatively=!1);if(canAnimateNatively){definition&&(keyframes$1=keyframes$1.map(value=>utils.isNumber(value)?definition.toDefaultUnit(value):value));1!==keyframes$1.length||featureDetection.supports.partialKeyframes()&&!isRecording||keyframes$1.unshift(readInitialValue());readInitialValue=
{delay:utils.time.ms(delay),duration:utils.time.ms(duration),endDelay:utils.time.ms(endDelay),easing:utils.isEasingList(easing$1)?void 0:easing.convertEasing(easing$1,duration),direction,iterations:repeat+1,fill:"both"};animation$1=element.animate({[name]:keyframes$1,offset,easing:utils.isEasingList(easing$1)?easing$1.map(thisEasing=>easing.convertEasing(thisEasing,duration)):void 0},readInitialValue);animation$1.finished||(animation$1.finished=new Promise((resolve,reject)=>{animation$1.onfinish=
resolve;animation$1.oncancel=reject}));const target=keyframes$1[keyframes$1.length-1];animation$1.finished.then(()=>{style.style.set(element,name,target);animation$1.cancel()}).catch(utils.noop);allowWebkitAcceleration||(animation$1.playbackRate=1.000001)}else valueIsTransform?(keyframes$1=keyframes$1.map(value=>"string"===typeof value?parseFloat(value):value),1===keyframes$1.length&&keyframes$1.unshift(parseFloat(readInitialValue())),animation$1=new animation.Animation(latest=>{definition&&(latest=
definition.toDefaultUnit(latest));style.style.set(element,name,latest)},keyframes$1,Object.assign(Object.assign({},options),{duration,easing:easing$1}))):(readInitialValue=keyframes$1[keyframes$1.length-1],style.style.set(element,name,definition&&utils.isNumber(readInitialValue)?definition.toDefaultUnit(readInitialValue):readInitialValue));isRecording&&record(element,key,keyframes$1,{duration,delay,easing:easing$1,repeat,offset},"motion-one");motionValue.setAnimation(animation$1);return animation$1}}}
